...
def draw_one_bbox(img, xyxy, label, color = (255,200,150), thickness=3, draw_mask = False, label_offset_x = 0, label_offset_y = -10):
  x1, y1, x2, y2 = xyxy
  if not draw_mask:
...
tab_detect_objs, tab_upload_db = st.tabs(["Detect Anything", "Create DB"])
with tab_detect_objs:
    with st.expander("Send Request"):
        ...
        submit = st.button(label="Process", key="button_submit_predict")
    ...
    if submit:
        ...
        with st.status("Processing data data...") as status:
            ...
            x = requests.post(
                f'{st.session_state["server_url"]}/predict',
                json = data,
                headers = {
...
with tab_upload_db: 
    data = st.file_uploader(
        "Sample Images", accept_multiple_files=True, key="db_samples"
    )
    submit = st.button(label="Process", key="button_submit_samples")
    if submit and data==[]:
